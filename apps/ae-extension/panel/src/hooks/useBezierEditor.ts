...
export function useBezierEditor(): BezierEditorState & {
  activePoint: 0 | 1 | 2 | 3 | null;
  setActivePoint: (index: 0 | 1 | 2 | 3 | null) => void;
} {
  const [controlPoints, setControlPoints] = useState<ControlPoints>([...DEFAULT_POINTS]);
  const [activePoint, setActivePoint] = useState<0 | 1 | 2 | 3 | null>(null);
  ...
  return { controlPoints, updateHandle, applyPreset, reset, activePoint, setActivePoint };
}
